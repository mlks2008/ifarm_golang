...
func (rl *RotateLogs) rotateNoLock(filename string) error {
	...
	if rl.linkName != "" {
		...
		if strings.Contains(rl.linkName, baseDir) {
			...
			if err != nil {
				return errors.New(fmt.Sprintf("failed to evaluate relative path from %#v to %#v", baseDir, rl.linkName))
			}
		...
		if err := os.Symlink(linkDest, tmpLinkName); err != nil {
			return errors.New("failed to create new symlink")
		}
		...
		if err != nil { // Assume err != nil means the directory doesn't exist
			if err := os.MkdirAll(linkDir, 0755); err != nil {
				return errors.New(fmt.Sprintf("failed to create directory %s", linkDir))
			}
		}

		if err := os.Rename(tmpLinkName, rl.linkName); err != nil {
			return errors.New("failed to rename new symlink")
		}
	}
