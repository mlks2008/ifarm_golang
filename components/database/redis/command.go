...
func (cmd *ScanCmd) setReply(val interface{}, err error) {
	defer func() {
		if err != nil {
			cmd.err = err
		}
	...
	arr, err := redis.Values(val, err)
	if err != nil {
		return
	}
	if len(arr) != 2 {
		err = fmt.Errorf("redis: got %d elements in scan reply, expected 2", len(arr))
		return
	}

	cmd.cursor, err = redis.Uint64(arr[0], nil)
	if err != nil {
		return
	}

	cmd.page, err = redis.Strings(arr[1], nil)
}
